...
public class PatientRecordGUI extends JFrame {
    ...
    public PatientRecordGUI() {
        ...
        String[] searchOptions = {"Patient ID", "Complaints", "Region"};
        ...
        switch (searchBy) {
            case "Patient ID":
                filteredRecords = records.stream()
                        .filter(record -> record.getPatientId().equalsIgnoreCase(query))
            ...
            case "Region":
                filteredRecords = records.stream()
                        .filter(record -> record.getRegion().equalsIgnoreCase(query))
                        .collect(Collectors.toList());
                break;
            default:
    ...
    private void updateTable(List<PatientRecord> records) {
        ...
        for (PatientRecord record : records) {
            Object[] rowData = {
                    record.getPatientId(), record.getName(), record.getAge(), record.getGender(),
                    record.getMedicalRecord(), record.getMedicalHistory(), record.getChiefComplaint(),
                    record.getHistoryOfParentIllness(), record.getPhysicalExamination(), record.getAssessmentAndPlan(),
                    record.getNhsTrustRegion(), record.getRegion()
            };
